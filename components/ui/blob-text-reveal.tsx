...
const travelDuration = (
  ...
  return Math.min(maxDuration, Math.max(minDuration, distancePx / speedPxPerSec));
};
...
export default function BlobTextReveal({
  ...
  const characters = useMemo(() => splitIntoCharacters(currentWord), [currentWord]);
  ...
  const getCharNodes = useCallback(() => {
    return charsRef.current.filter((node): node is HTMLSpanElement => node != null);
  }, []);
  ...
  const measureLayout = useCallback((): Layout | null => {
    ...
    if (chars.some((c) => !Number.isFinite(c.center) || c.right <= c.left)) return null;
    ...
    const clampX = (value: number) => Math.min(maxX, Math.max(-size * 0.25, value));

    return { chars, homeX: clampX(homeX), leftX: clampX(leftX), nodes };
  }, [getCharNodes]);
  ...
  useLayoutEffect(() => {
    if (suppressAutoParkRef.current) { setBlobReady(true); return; }
  ...
  useEffect(() => {
    ...
    const wait = (ms: number) => new Promise<void>((resolve) => { holdTimer = setTimeout(resolve, ms); });

    const setBlobColorSmooth = (next: string, instant = false) => {
      colorAnim?.stop();
      if (instant) { blobColor.set(next); return; }
      colorAnim = animate(blobColor, next, { duration: BLOB_COLOR_DURATION, ease: BLOB_COLOR_EASE });
    };

    const blobHalf = () => blobWidth.get() / 2;

    const resetBlobScale = () => { deformAnim?.stop(); blobWidth.set(blobSizeRef.current); };
    ...
    const setBlobDeform = (mode: "wipe" | "rest") => {
      ...
      return new Promise<void>((resolve) => {
        deformAnim = animate(blobWidth, target, { duration: DEFORM_DURATION, ease: DEFORM_EASE, onComplete: () => resolve() });
      });
    };

    const hideCharsWithBlob = (nodes: HTMLSpanElement[], chars: CharMetrics[], x: number) => {
      const half = blobHalf();
    ...
    const revealChar = (node: HTMLSpanElement) => {
      charAnim = animate(
        node,
        { opacity: [0, 1], y: [START_Y, 0], filter: [`blur(${blur}px)`, "blur(0px)"] } as any,
        { duration: CHAR_REVEAL_DURATION, ease: [0.215, 0.61, 0.355, 1] }
      );
    };

    const revealWithBlob = (nodes: HTMLSpanElement[], chars: CharMetrics[], fromX: number, homeX: number) =>
      new Promise<void>((resolve) => {
        ...
        const blobDuration = travelDuration(distance, REVEAL_SPEED_PX_PER_SEC, MIN_REVEAL_DURATION, MAX_REVEAL_DURATION);
        activeAnim = animate(blobLeft, homeX, {
          duration: blobDuration, ease: "linear",
          onUpdate: (x) => {
            const half = blobHalf();
            const leadEdge = x + half;
            nodes.forEach((node, i) => {
              ...
              const metrics = chars[i];
              if (!metrics) return;
              if (leadEdge >= metrics.left) { revealed.add(i); revealChar(node); }
            });
          },
          onComplete: () => {
            nodes.forEach((node, i) => { if (!revealed.has(i)) { revealed.add(i); revealChar(node); } });
            holdTimer = setTimeout(() => { setCharsVisible(nodes); resolve(); }, POST_PARK_SETTLE * 1000);
          },
        });
      });
    ...
    const runLoop = async () => {
      ...
      while (!cancelled) {
        ...
        const wipeDurationSec = travelDuration(wipeDistance, WIPE_SPEED_PX_PER_SEC, MIN_WIPE_DURATION, MAX_WIPE_DURATION);
        let shrinkStarted = false;
        await new Promise<void>((resolve) => {
          activeAnim = animate(blobLeft, wipeLayout.leftX, {
            duration: wipeDurationSec, ease: "linear",
            onUpdate: (x) => {
              ...
              if (!shrinkStarted && wipeDistance > 0) {
                ...
                if (progress >= 0.82) { shrinkStarted = true; void setBlobDeform("rest"); setBlobColorSmooth(revealColorRef.current); }
              }
            },
            onComplete: () => { if (!shrinkStarted) { void setBlobDeform("rest"); setBlobColorSmooth(revealColorRef.current); } resolve(); },
          });
        });
        if (cancelled) break;
        ...
        layout = await waitForLayout();
        if (!layout || cancelled) break;
        layout.nodes.forEach((node) => { node.style.transform = "translateY(0px)"; node.style.opacity = "0"; node.style.filter = `blur(${blur}px)`; });
        await new Promise<void>((resolve) => { window.requestAnimationFrame(() => resolve()); });
        ...
        await revealWithBlob(revealLayout.nodes, revealLayout.chars, revealLayout.leftX, revealLayout.homeX);
        if (cancelled) break;
    ...
    return () => {
      ...
      activeAnim?.stop(); deformAnim?.stop(); charAnim?.stop(); colorAnim?.stop();
      resetBlobScale();
    };
  }, [safeTexts, holdDuration, blur, blobLeft, blobWidth, blobColor, waitForLayout]);

  useEffect(() => { animate(blobColor, revealColor, { duration: BLOB_COLOR_DURATION, ease: BLOB_COLOR_EASE }); }, [revealColor, blobColor]);
  ...
  useEffect(() => {
    ...
    if (nearHome) blobLeft.set(layout.homeX);
    else if (nearLeft) blobLeft.set(layout.leftX);
  }, [safeBlobSize, blobLeft, blobWidth]);

  const textAlign = (font.textAlign as React.CSSProperties["textAlign"]) ?? "center";
  const justifyContent = textAlign === "center" ? "center" : textAlign === "right" || textAlign === "end" ? "flex-end" : "flex-start";

  return (
    <div style={{ ...font, width: "100%", height: "100%", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent, gap: "0.35em", textAlign, ...style }}>
      {prefix ? <span style={{ color: prefixColor, whiteSpace: "pre" }}>{prefix}</span> : null}
      <span ref={wrapperRef} style={{ position: "relative", display: "inline-block", color, letterSpacing: font.letterSpacing ?? "0em", lineHeight: font.lineHeight ?? 1.1, verticalAlign: "baseline", paddingRight: Math.max(6, Math.round(safeBlobSize * 0.45)) + safeBlobSize }}>
        <span aria-hidden="true" style={{ display: "inline-block", whiteSpace: "pre" }}>
          {characters.map((char, i) => (
            <span key={`${currentWord}-${i}`} ref={(node) => { charsRef.current[i] = node; }} className="char" style={{ display: "inline-block", transformOrigin: "50% 50%", willChange: "transform, opacity, filter" }}>
              {char === " " ? "\u00A0" : char}
            </span>
          ))}
        </span>
        ...
        <motion.span
          aria-hidden="true"
          style={{
            position: "absolute", bottom: "0.08em", left: 0, x: blobLeft, width: blobWidth, height: safeBlobSize,
            marginLeft: blobMarginLeft, marginBottom: blobMarginBottom, borderRadius: 9999,
            backgroundColor: blobColor, display: "block", pointerEvents: "none",
            opacity: blobReady ? 1 : 0, willChange: "transform, width, background-color",
          }}
        />
      </span>
    </div>
