...
function MeshTextBase(props: Record<string, unknown>) {
  const merged = { ...COMPONENT_DEFAULTS, ...props }
  const { text, color, font, colorSplit, customColors, force } = merged
  ...
  const fontStyle = typeof font?.fontStyle === "string"
    ? font.fontStyle as string
    : variantIsItalic(fontVariant) ? "italic" : "normal"
  ...
  useEffect(() => {
    ...
    const rebuildTex = async () => {
      ...
      const c2 = renderTextToCanvas(
        String(text ?? ""), color ?? "#ffffff", fontFamily,
        fontWeight, fontStyle, realSize, w, h
      )
      gl.bindTexture(gl.TEXTURE_2D, tex)
    ...
    const tick = () => {
      ...
      if (cols.length === 1) { cA = cols[0]; cB = cols[0] }
        ...
        cA = cols[ci]; cB = cols[(ci + 1) % cols.length]
      }
...
export default function MeshText(props: Record<string, unknown>) {
  return <MeshTextBase {...(__originkitPresetProps as Record<string, unknown>)} {...props} />
}
