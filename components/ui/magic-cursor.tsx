...
export default function MagicCursor({
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (!seen || !inside) { x = targetX; y = targetY; vx = 0; vy = 0; seen = true; }
      inside = true;
    };
    const onWindowLeave = () => { inside = false; hideNativeCursor(false); };
  ...
  const arrow = (
    <svg width={ARROW_W} height={ARROW_H} viewBox={`0 0 ${ARROW_W} ${ARROW_H}`} style={{ display: "block", overflow: "visible" }} aria-hidden>
      <path d={ARROW} fill={fillColor} stroke="rgba(0,0,0,0.18)" strokeWidth={0.6} strokeLinejoin="round" />
    </svg>
  );

  const labelNode = label ? (
    <div style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", whiteSpace: "pre", pointerEvents: "none", userSelect: "none", ...resolvedLabelFont, color: labelColor }}>
      {labelText}
    </div>
  ) : null;

  return (
    <div ref={frameRef} style={{ position: "fixed", inset: 0, overflow: "hidden", pointerEvents: "none", zIndex: 9999, ...style }}>
      {labelNode}
      <div ref={hostRef} style={{ position: "absolute", top: 0, left: 0, transformOrigin: "0 0", opacity: 0, pointerEvents: "none", willChange: "transform", filter: glowFilter }}>
        {arrow}
      </div>
    </div>
  );
